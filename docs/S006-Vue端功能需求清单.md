...
> 不使用 ABP 的 `@abp/ng.oauth`（Angular 专用），改用通用 OIDC 客户端库对接 ABP 后端的 OpenIddict。
...
- 工具栏：`+ 新建用户` 按钮
- 行操作：编辑、删除、权限管理
- 可通过注入令牌扩展列表列（`IDENTITY_ENTITY_PROP_CONTRIBUTORS` 模式）
...
> 可通过注入令牌扩展表单字段（`IDENTITY_CREATE/EDIT_FORM_PROP_CONTRIBUTORS`）。
...
| 场景 | 行为 |
|------|------|
| 全页面加载 | 顶部 `abp-loader-bar`（NProgress） |
...
| 后端不可达 | 全局离线指示器（类似 Angular `abp-internet-status`） |
...
| 可见标签页过滤 | `visible$` 动态控制：无权限或无内容的标签页自动隐藏 |

### 10.10 全局导航与菜单

| 需求 | 说明 |
|------|------|
| 动态侧边菜单 | 从 `RoutesService` 获取菜单树，按权限过滤，支持分组折叠 |
