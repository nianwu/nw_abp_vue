...
将 OIDC 认证逻辑从 `plugins/oidc.ts` 和 `stores/auth.ts` 中抽离为独立插件配置，支持运行时切换认证方式（OIDC / 自定义登录 / 无认证）。目标：`createApp().use(oidcPlugin, config)` 式声明。

### 15.2 数据提供层可插拔

当前 `providers/remote/` 和 `providers/standalone/` 通过 `VITE_PROVIDER_MODE` 环境变量切换。规划将数据提供层抽象为统一接口，remote 和 standalone 作为可选项按需引入，便于新增 mock-provider、graphql-provider 等后端接入方式。

### 15.3 ABP 逻辑收敛为可选插件包

将 `types/abp.ts`、`utils/abp-error.ts`、`utils/permission-modal.ts`、`utils/feature-modal.ts`、`stores/app-config.ts` 等 ABP 耦合逻辑从核心代码中抽离，收敛为独立的 `@nw/abp-vue` 插件包。非 ABP 项目可完全不引入。

### 15.4 DDD 模式抽象

以领域驱动设计的分层思想重构项目结构：

- **Domain 层**：实体类型、值对象、repository 接口
- **Application 层**：use case / service 编排
- **Infrastructure 层**：API 调用、持久化、认证实现
- **Presentation 层**：Vue 组件、路由、视图

目标：核心业务逻辑与 UI 框架解耦，提升可测试性和扩展能力。

### 15.5 i18n 可选项

将国际化从核心依赖变为可选插件——不引入 i18n 时项目以默认中文运行，引入后按需加载语言包，支持运行时语言切换。降低非国际化项目的包体积和配置成本。

### 15.6 主题切换与设置

加入亮色/暗色主题切换能力，利用 Element Plus CSS 变量 + Tailwind 暗色模式类实现。扩展 `SettingsView` 或独立设置面板，支持用户偏好持久化（localStorage / 后端用户配置）。
