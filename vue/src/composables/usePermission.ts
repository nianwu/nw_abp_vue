...
export function usePermission() {
  ...
  function hasPermission(policy: string): boolean {
    if (!appConfig.config?.auth?.grantedPolicies) return false
    return appConfig.config.auth.grantedPolicies[policy] === true
  }

  /** 批量检查，全部满足返回 true */
  function hasAllPermissions(policies: string[]): boolean {
    return policies.every((p) => hasPermission(p))
  }

  /** 批量检查，任一满足返回 true */
  function hasAnyPermission(policies: string[]): boolean {
...
export function permissionGuard(policy?: string): boolean {
