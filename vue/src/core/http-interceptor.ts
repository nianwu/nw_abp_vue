...
import { standaloneGetPermissions, standaloneUpdatePermission } from '@/stores/permission-store'
import { standaloneGetFeatures, standaloneUpdateFeatureValue } from '@/stores/feature-store'
import {
...
function matchStore(method: string, url: string, params: any, data: any): any | undefined {
  ...
  if (method === 'POST' && urlWithoutQuery === '/api/setting-management/timezone') {
    ...
    if (tzData?.timezone) standaloneSetTimezone(tzData.timezone)
    return EMPTY_OK
  }
