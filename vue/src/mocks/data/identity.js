...
export function mockGetUsers(params) {
    ...
    let filtered = users.filter(u => !filter || (u.userName || '').toLowerCase().includes(filter.toLowerCase()) || (u.email || '').toLowerCase().includes(filter.toLowerCase()));
...
export function mockUpdateUser(id, data) {
...
export function mockGetUserRoles(_id) {
...
let roleSeq = 5;
export function mockGetRoles(params) {
    const { filter = '', skipCount = 0, maxResultCount = 10 } = params || {};
...
export function mockGetAllRoles() {
    return { items: [...roles] };
}
export function mockGetRole(id) {
    return roles.find(r => r.id === id);
}
export function mockCreateRole(data) {
...
export function mockUpdateRole(id, data) {
